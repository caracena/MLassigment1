...
\emph{Lecturer:} \\
 \textsc{Fabio Ramos} % Supervisor's Name
\end{flushright}
\end{minipage}\\[4cm]
...
%image code
\begin{figure}[H]
\includegraphics[width=\textwidth]{university-of-sydney-logo}
\centering
\caption{Caption}
\label{fig:1}
\end{figure}
...
%equation
\begin{equation*}
Precision\ P\ = \frac{|G \cap S |}{|S|} \text{,}\qquad Recall\ R\ = \frac{|G \cap S |}{|G|} \text{,}\qquad F \dash score\ F_{1}\ = \frac{2PR}{P+R} 
\end{equation*}
...
\hline
\textbf{Categories} & \textbf{Precision} & \textbf{Recall} & \textbf{F-score} \\ \hline
\end{tabular}
\caption{Precision, Recall, F-score}
\end{table}

%cite
reference~\cite{wiki:pandr}
